...
class AnalysisConfig(BaseModel):
    ...
    #: 短于这个时长的高亮段不单独标出来，直接并回两侧的保留段。
    #:
    #: 这条不是审美问题。两个下沿不同的刀切出来的边界能差到一个采样点（62.5µs），
    #: 实测在「标准」档就会留下 0.00s 的高亮段和同样短的保留段碎片；而高亮段是要
    #: 用户逐段听过去判断的，62µs 听不出任何东西，跳转导航落上去却什么也看不到。
    #: 并回两侧之后内容照旧保留（只是不高亮），所以这条过滤不会造成误剪。
    #:
    #: 默认 250 和 vad_min_speech_duration_ms 同值：短于 250ms 的东西，检测那边
    #: 已经不当作一段人声了，复核这边也不值得请用户判断一次。
    vad_uncertain_min_duration_ms: int = Field(default=250, ge=0)
